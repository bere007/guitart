...
6. **Authentication → Email Templates** — по умолчанию Supabase требует подтверждение почты после регистрации; это нормально, можно отключить в Settings → Auth, если хочешь мгновенный вход без письма.
...
site/                   статический фронтенд (деплоится на GitHub Pages)
  ...
  course.html              личный кабинет ученика
  payment.html              оплата
  exam.html                  запись на экзамен + сертификат
